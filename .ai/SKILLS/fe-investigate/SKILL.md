---
name: fe-investigate
description: Run investigate for frontend work only. Use the FE lane source of truth, update FE docs, and record BE dependencies as cross-lane handoff instead of mixing scope.
---
...
Run the `investigate` workflow in the FE lane: collect evidence and isolate root cause inside the lane before proposing a fix.

Base skill intent: Run a disciplined investigation to collect evidence, isolate hypotheses, and identify the real fault line before proposing a fix.

## when to use

- When the user invokes `/fe-investigate`.
...
- `.ai/LANES.md`
- `.ai/DOCS.md`
- `.ai/SKILLS/investigate/SKILL.md` for the base stage workflow
...
3. Read `.ai/SKILLS/investigate/SKILL.md` and apply its procedure inside the FE lane boundary.
...
- FE-scoped result for `investigate`
...
- Hand off to the unprefixed `investigate` skill when the work is intentionally cross-functional.
