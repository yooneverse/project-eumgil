---
name: fe-document-release
description: Run document-release for frontend work only. Use the FE lane source of truth, update FE docs, and record BE dependencies as cross-lane handoff instead of mixing scope.
---
...
Run the `document-release` workflow in the FE lane: update lane-specific release and source documentation.

Base skill intent: Update release-facing docs so the repository tells the truth about what just shipped and how to operate it.

## when to use

- When the user invokes `/fe-document-release`.
...
- `.ai/LANES.md`
- `.ai/DOCS.md`
- `.ai/SKILLS/document-release/SKILL.md` for the base stage workflow
...
3. Read `.ai/SKILLS/document-release/SKILL.md` and apply its procedure inside the FE lane boundary.
...
- FE-scoped result for `document-release`
...
- Hand off to the unprefixed `document-release` skill when the work is intentionally cross-functional.
