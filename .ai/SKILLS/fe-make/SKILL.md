---
name: fe-make
description: Run make for frontend work only. Use the FE lane source of truth, update FE docs, and record BE dependencies as cross-lane handoff instead of mixing scope.
---
...
Run the `make` workflow in the FE lane: frame the problem, user, scope, and non-goals before planning.

Base skill intent: Turn a vague idea into a sharper problem definition, wedge, target user, and non-goals before planning starts.

## when to use

- When the user invokes `/fe-make`.
...
- `.ai/LANES.md`
- `.ai/DOCS.md`
- `.ai/SKILLS/make/SKILL.md` for the base stage workflow
...
3. Read `.ai/SKILLS/make/SKILL.md` and apply its procedure inside the FE lane boundary.
...
- Hand off to the unprefixed `make` skill when the work is intentionally cross-functional.
