---
name: be-deploy-check
description: Run deploy-check for backend work only. Use the BE lane source of truth, update BE docs, and record FE dependencies as cross-lane handoff instead of mixing scope.
---
...
Run the `deploy-check` workflow in the BE lane: confirm lane-specific deployment prerequisites and rollback assumptions.
...
- When the user invokes `/be-deploy-check`.
...
3. Read `.ai/SKILLS/deploy-check/SKILL.md` and apply its procedure inside the BE lane boundary.
...
- BE-scoped result for `deploy-check`
