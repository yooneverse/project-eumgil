---
name: be-plan-eng-review
description: Run plan-eng-review for backend work only. Use the BE lane source of truth, update BE docs, and record FE dependencies as cross-lane handoff instead of mixing scope.
---
...
Run the `plan-eng-review` workflow in the BE lane: review architecture, contracts, data flow, failure modes, and tests for this lane.
...
- When the user invokes `/be-plan-eng-review`.
...
3. Read `.ai/SKILLS/plan-eng-review/SKILL.md` and apply its procedure inside the BE lane boundary.
...
- BE-scoped result for `plan-eng-review`
