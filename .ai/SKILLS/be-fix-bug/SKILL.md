---
name: be-fix-bug
description: Run fix-bug for backend work only. Use the BE lane source of truth, update BE docs, and record FE dependencies as cross-lane handoff instead of mixing scope.
---
...
Run the `fix-bug` workflow in the BE lane: reproduce and fix a lane-specific defect with regression coverage.
...
- When the user invokes `/be-fix-bug`.
...
3. Read `.ai/SKILLS/fix-bug/SKILL.md` and apply its procedure inside the BE lane boundary.
