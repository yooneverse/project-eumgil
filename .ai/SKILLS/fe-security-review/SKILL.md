...
description: Run security-review for frontend work only. Use the FE lane source of truth, update FE docs, and record BE dependencies as cross-lane handoff instead of mixing scope.
---
...
Run the `security-review` workflow in the FE lane: review lane-specific trust boundaries, data exposure, auth, and secrets risk.
...
- When the user invokes `/fe-security-review`.
...
3. Read `.ai/SKILLS/security-review/SKILL.md` and apply its procedure inside the FE lane boundary.
...
- FE-scoped result for `security-review`
