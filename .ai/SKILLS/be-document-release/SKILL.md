---
name: be-document-release
description: Run document-release for backend work only. Use the BE lane source of truth, update BE docs, and record FE dependencies as cross-lane handoff instead of mixing scope.
---
...
Run the `document-release` workflow in the BE lane: update lane-specific release and source documentation.
...
- When the user invokes `/be-document-release`.
...
3. Read `.ai/SKILLS/document-release/SKILL.md` and apply its procedure inside the BE lane boundary.
...
- BE-scoped result for `document-release`
