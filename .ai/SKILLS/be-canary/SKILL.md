---
name: be-canary
description: Run canary for backend work only. Use the BE lane source of truth, update BE docs, and record FE dependencies as cross-lane handoff instead of mixing scope.
---
...
Run the `canary` workflow in the BE lane: watch lane-specific release health immediately after deploy.
...
- When the user invokes `/be-canary`.
...
3. Read `.ai/SKILLS/canary/SKILL.md` and apply its procedure inside the BE lane boundary.
