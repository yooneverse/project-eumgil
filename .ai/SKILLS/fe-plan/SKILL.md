---
name: fe-plan
description: Run plan for frontend work only. Use the FE lane source of truth, update FE docs, and record BE dependencies as cross-lane handoff instead of mixing scope.
---
...
Run the `plan` workflow in the FE lane: produce an executable lane-specific plan with checklist, source docs, validation, and handoff notes.

Base skill intent: Run a full planning pass by consolidating problem framing, product scope review, engineering review, and design review into one reusable sprint artifact.

## when to use

- When the user invokes `/fe-plan`.
...
- `.ai/LANES.md`
- `.ai/DOCS.md`
- `.ai/SKILLS/plan/SKILL.md` for the base stage workflow
...
3. Read `.ai/SKILLS/plan/SKILL.md` and apply its procedure inside the FE lane boundary.
...
- Hand off to the unprefixed `plan` skill when the work is intentionally cross-functional.
