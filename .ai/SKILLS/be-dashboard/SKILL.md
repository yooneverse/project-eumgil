---
name: be-dashboard
description: Run dashboard for backend work only. Use the BE lane source of truth, update BE docs, and record FE dependencies as cross-lane handoff instead of mixing scope.
---
...
Run the `dashboard` workflow in the BE lane: show lane-specific checklist progress and next command guidance.

Base skill intent: Show a Korean checklist-based dashboard from the local plan file.

## when to use

- When the user invokes `/be-dashboard`.
...
3. Read `.ai/SKILLS/dashboard/SKILL.md` and apply its procedure inside the BE lane boundary.
...
- BE-scoped result for `dashboard`
