---
name: be-qa-only
description: Run qa-only for backend work only. Use the BE lane source of truth, update BE docs, and record FE dependencies as cross-lane handoff instead of mixing scope.
---
...
Run the `qa-only` workflow in the BE lane: produce a lane-specific QA report without code changes.

Base skill intent: Run the same real-flow validation as QA but stop at reporting instead of changing code.

## when to use

- When the user invokes `/be-qa-only`.
...
3. Read `.ai/SKILLS/qa-only/SKILL.md` and apply its procedure inside the BE lane boundary.
