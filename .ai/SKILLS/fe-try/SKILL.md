---
name: fe-try
description: Run try for frontend work only. Use the FE lane source of truth, update FE docs, and record BE dependencies as cross-lane handoff instead of mixing scope.
---
...
Run the `try` workflow in the FE lane: close the lane-specific sprint loop and capture process improvements.

Base skill intent: Close the sprint loop by summarizing what worked, what broke, what slowed the team down, and what should change in the harness.

## when to use

- When the user invokes `/fe-try`.
...
- `.ai/LANES.md`
- `.ai/DOCS.md`
- `.ai/SKILLS/try/SKILL.md` for the base stage workflow
...
3. Read `.ai/SKILLS/try/SKILL.md` and apply its procedure inside the FE lane boundary.
...
- Hand off to the unprefixed `try` skill when the work is intentionally cross-functional.
