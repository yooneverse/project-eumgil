---
name: fe-deploy-check
description: Run deploy-check for frontend work only. Use the FE lane source of truth, update FE docs, and record BE dependencies as cross-lane handoff instead of mixing scope.
---
...
Run the `deploy-check` workflow in the FE lane: confirm lane-specific deployment prerequisites and rollback assumptions.

Base skill intent: Confirm deployment prerequisites, environment assumptions, and rollback readiness before pushing a release over the line.

## when to use

- When the user invokes `/fe-deploy-check`.
...
- `.ai/LANES.md`
- `.ai/DOCS.md`
- `.ai/SKILLS/deploy-check/SKILL.md` for the base stage workflow
...
3. Read `.ai/SKILLS/deploy-check/SKILL.md` and apply its procedure inside the FE lane boundary.
...
- FE-scoped result for `deploy-check`
...
- Hand off to the unprefixed `deploy-check` skill when the work is intentionally cross-functional.
