---
name: be-write-test
description: Run write-test for backend work only. Use the BE lane source of truth, update BE docs, and record FE dependencies as cross-lane handoff instead of mixing scope.
---
...
Run the `write-test` workflow in the BE lane: add focused tests for lane-specific behavior and contracts.
...
- When the user invokes `/be-write-test`.
...
3. Read `.ai/SKILLS/write-test/SKILL.md` and apply its procedure inside the BE lane boundary.
...
- BE-scoped result for `write-test`
