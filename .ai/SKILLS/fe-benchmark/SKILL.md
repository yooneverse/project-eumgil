---
name: fe-benchmark
description: Run benchmark for frontend work only. Use the FE lane source of truth, update FE docs, and record BE dependencies as cross-lane handoff instead of mixing scope.
---
...
Run the `benchmark` workflow in the FE lane: measure lane-specific performance or resource behavior.

Base skill intent: Capture performance baselines and before-versus-after comparisons so release decisions are not made on intuition alone.

## when to use

- When the user invokes `/fe-benchmark`.
...
- `.ai/LANES.md`
- `.ai/DOCS.md`
- `.ai/SKILLS/benchmark/SKILL.md` for the base stage workflow
...
3. Read `.ai/SKILLS/benchmark/SKILL.md` and apply its procedure inside the FE lane boundary.
...
- FE-scoped result for `benchmark`
...
- Hand off to the unprefixed `benchmark` skill when the work is intentionally cross-functional.
