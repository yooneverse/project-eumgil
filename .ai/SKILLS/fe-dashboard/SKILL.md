---
name: fe-dashboard
description: Run dashboard for frontend work only. Use the FE lane source of truth, update FE docs, and record BE dependencies as cross-lane handoff instead of mixing scope.
---
...
Run the `dashboard` workflow in the FE lane: show lane-specific checklist progress and next command guidance.

Base skill intent: Show a Korean checklist-based dashboard from the local plan file.

## when to use

- When the user invokes `/fe-dashboard`.
...
- `.ai/LANES.md`
- `.ai/DOCS.md`
- `.ai/SKILLS/dashboard/SKILL.md` for the base stage workflow
...
3. Read `.ai/SKILLS/dashboard/SKILL.md` and apply its procedure inside the FE lane boundary.
...
- FE-scoped result for `dashboard`
...
- Hand off to the unprefixed `dashboard` skill when the work is intentionally cross-functional.
