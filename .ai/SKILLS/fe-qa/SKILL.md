---
name: fe-qa
description: Run qa for frontend work only. Use the FE lane source of truth, update FE docs, and record BE dependencies as cross-lane handoff instead of mixing scope.
---
...
Run the `qa` workflow in the FE lane: validate lane-specific real user or API flows.

Base skill intent: Test real user flows, document what breaks, and produce a bug and risk report that reflects actual product usage.

## when to use

- When the user invokes `/fe-qa`.
...
- `.ai/LANES.md`
- `.ai/DOCS.md`
- `.ai/SKILLS/qa/SKILL.md` for the base stage workflow
...
3. Read `.ai/SKILLS/qa/SKILL.md` and apply its procedure inside the FE lane boundary.
...
- Hand off to the unprefixed `qa` skill when the work is intentionally cross-functional.
