---
name: fe-write-test
description: Run write-test for frontend work only. Use the FE lane source of truth, update FE docs, and record BE dependencies as cross-lane handoff instead of mixing scope.
---
...
Run the `write-test` workflow in the FE lane: add focused tests for lane-specific behavior and contracts.

Base skill intent: Add focused tests for behavior, edge cases, and failure modes that the current suite does not cover well enough.

## when to use

- When the user invokes `/fe-write-test`.
...
- `.ai/LANES.md`
- `.ai/DOCS.md`
- `.ai/SKILLS/write-test/SKILL.md` for the base stage workflow
...
3. Read `.ai/SKILLS/write-test/SKILL.md` and apply its procedure inside the FE lane boundary.
...
- FE-scoped result for `write-test`
...
- Hand off to the unprefixed `write-test` skill when the work is intentionally cross-functional.
