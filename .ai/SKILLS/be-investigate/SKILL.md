---
name: be-investigate
description: Run investigate for backend work only. Use the BE lane source of truth, update BE docs, and record FE dependencies as cross-lane handoff instead of mixing scope.
---
...
Run the `investigate` workflow in the BE lane: collect evidence and isolate root cause inside the lane before proposing a fix.
...
- When the user invokes `/be-investigate`.
...
3. Read `.ai/SKILLS/investigate/SKILL.md` and apply its procedure inside the BE lane boundary.
...
- BE-scoped result for `investigate`
