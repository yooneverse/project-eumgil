---
name: fe-canary
description: Run canary for frontend work only. Use the FE lane source of truth, update FE docs, and record BE dependencies as cross-lane handoff instead of mixing scope.
---
...
Run the `canary` workflow in the FE lane: watch lane-specific release health immediately after deploy.

Base skill intent: Verify a release in the first moments after deployment by watching the most likely breakpoints and user-critical signals.

## when to use

- When the user invokes `/fe-canary`.
...
- `.ai/LANES.md`
- `.ai/DOCS.md`
- `.ai/SKILLS/canary/SKILL.md` for the base stage workflow
...
3. Read `.ai/SKILLS/canary/SKILL.md` and apply its procedure inside the FE lane boundary.
...
- Hand off to the unprefixed `canary` skill when the work is intentionally cross-functional.
