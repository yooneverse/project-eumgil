---
name: be-benchmark
description: Run benchmark for backend work only. Use the BE lane source of truth, update BE docs, and record FE dependencies as cross-lane handoff instead of mixing scope.
---
...
Run the `benchmark` workflow in the BE lane: measure lane-specific performance or resource behavior.
...
- When the user invokes `/be-benchmark`.
...
3. Read `.ai/SKILLS/benchmark/SKILL.md` and apply its procedure inside the BE lane boundary.
...
- BE-scoped result for `benchmark`
