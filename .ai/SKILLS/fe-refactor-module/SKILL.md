---
name: fe-refactor-module
description: Run refactor-module for frontend work only. Use the FE lane source of truth, update FE docs, and record BE dependencies as cross-lane handoff instead of mixing scope.
---
...
Run the `refactor-module` workflow in the FE lane: improve lane-specific structure while preserving behavior.

Base skill intent: Improve module structure, boundaries, or readability while preserving behavior and making architectural intent clearer.

## when to use

- When the user invokes `/fe-refactor-module`.
...
3. Read `.ai/SKILLS/refactor-module/SKILL.md` and apply its procedure inside the FE lane boundary.
...
- FE-scoped result for `refactor-module`
...
- Hand off to the unprefixed `refactor-module` skill when the work is intentionally cross-functional.
