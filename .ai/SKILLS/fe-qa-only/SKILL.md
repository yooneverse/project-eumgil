---
name: fe-qa-only
description: Run qa-only for frontend work only. Use the FE lane source of truth, update FE docs, and record BE dependencies as cross-lane handoff instead of mixing scope.
---
...
Run the `qa-only` workflow in the FE lane: produce a lane-specific QA report without code changes.

Base skill intent: Run the same real-flow validation as QA but stop at reporting instead of changing code.

## when to use

- When the user invokes `/fe-qa-only`.
...
- `.ai/LANES.md`
- `.ai/DOCS.md`
- `.ai/SKILLS/qa-only/SKILL.md` for the base stage workflow
...
3. Read `.ai/SKILLS/qa-only/SKILL.md` and apply its procedure inside the FE lane boundary.
...
- Hand off to the unprefixed `qa-only` skill when the work is intentionally cross-functional.
