---
name: be-start
description: Run start for backend work only. Use the BE lane source of truth, update BE docs, and record FE dependencies as cross-lane handoff instead of mixing scope.
---
...
Run the `start` workflow in the BE lane: implement the approved plan without crossing into the opposite lane.

Base skill intent: Build an approved feature against the recorded plan instead of improvising from the latest prompt.

## when to use

- When the user invokes `/be-start`.
...
3. Read `.ai/SKILLS/start/SKILL.md` and apply its procedure inside the BE lane boundary.
